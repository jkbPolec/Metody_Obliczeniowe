...
# ==============================================================================
# NOWA FUNKCJA DO GENEROWANIA INDYWIDUALNYCH WYKRESÓW BŁĘDU
# ==============================================================================
def generate_individual_error_plot(scheme, solver):
    """
    Generuje osobny wykres błędu w funkcji czasu dla podanej
    kombinacji metody i solvera.
    """
    ...
    title = f"Błąd w funkcji czasu dla {LABEL_MAP[scheme]} + {LABEL_MAP[solver]}"
    ax.set_title(title, fontsize=16)
    ax.set_xlabel("Czas t")
    ax.set_ylabel("Maksymalny błąd bezwzględny")
    ax.grid(True, which="both", ls="--")
    # Skala logarytmiczna jest przydatna, aby zobaczyć szczegóły małych błędów
    ax.set_yscale('log')

    output_filename = f'wykres_3a_blad_indywidualny_{scheme}_{solver}.png'
    plt.savefig(output_filename, dpi=300, bbox_inches='tight')
    plt.close(fig)
    print(f"Zapisano indywidualny wykres błędu: {output_filename}")
...
def plot_evolution_comparison_all_methods():
    ...
    axes = axes.flatten()
    styles = {
        'Laasonen_Thomas': {'color': 'blue', 'marker': 'o'}, 'Laasonen_LU': {'color': 'cyan', 'marker': 'x'},
        'CN_Thomas': {'color': 'red', 'marker': 'o'}, 'CN_LU': {'color': 'magenta', 'marker': 'x'}
    }
    for i, (t, cols) in enumerate(snapshots.items()):
        ...
        for scheme in SCHEMES:
            for solver in SOLVERS:
                ...
                style_key = f"{scheme}_{solver}"; label = f'{LABEL_MAP[scheme]} + {LABEL_MAP[solver]}'
                ax.plot(data_num[:, 0], data_num[:, cols[0]], 'None', marker=styles[style_key]['marker'],
                        color=styles[style_key]['color'], markersize=5, label=label, alpha=0.8)
...
def plot_error_vs_time_comparison_all_methods():
    ...
    ax.set_title('Zbiorcze porównanie błędu w funkcji czasu (dla dużej λ)', fontsize=16)
    ax.set_xlabel('Czas t'); ax.set_ylabel('Maksymalny błąd bezwzględny')
    ax.set_yscale('log'); ax.legend()
    output_filename = 'wykres_3b_porownanie_bledow.png'
    plt.savefig(output_filename, dpi=300, bbox_inches='tight'); plt.close(fig)
    print(f"Zapisano zbiorczy wykres porównawczy błędów: {output_filename}")
